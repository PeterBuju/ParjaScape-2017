...
public class CombatAssistant{
	...
	*/
		...
		if (Server.npcHandler.npcs[i] != null) {
			...
			if(!c.castingMagic && c.projectileStage > 0) { // range hit damage
				int damage = Misc.random(rangeMaxHit());
                                ...
                                if (CalculateCriticalChance() && damage > 0){
                                    sendCriticalDamageGloating("Ranged");
                                    damage = CalculateCriticalDamage(damage, "Ranged");
                                    critical = true;
                                }
				int damage2 = -1;
				...
				if(Misc.random(Server.npcHandler.npcs[i].defence) > Misc.random(10+calculateRangeAttack()) && !ignoreDef && !critical) {
					damage = 0;
	...
	public void applyNpcMeleeDamage(int i, int damageMask) {
		int damage = Misc.random(calculateMeleeMaxHit());
                boolean critical = false;
                if (CalculateCriticalChance() && damage != 0){
                        sendCriticalDamageGloating("Melee");
                        damage = CalculateCriticalDamage(damage, "Melee");
                        critical = true;
                }
		...
		if (!fullVeracsEffect) {
			if (Misc.random(Server.npcHandler.npcs[i].defence) > 10 + Misc.random(calculateMeleeAttack()) && !critical) {
				damage = 0;
	...
	public void getPlayerAnimIndex(String weaponName){
		...
		if(weaponName.contains("halberd") || weaponName.contains("guthan")) {
			...
			return;
                ...
                else if(weaponName.contains("2h sword") || weaponName.contains("godsword") || weaponName.contains("saradomin sw")) {
	...
	**/
	...
	public boolean armaNpc(int i) {
        ...
        public boolean CalculateCriticalChance(){
            if (c.perks.contains(PerkHandler.GetPerk("Critical Strike"))){
                int chance = Misc.random(100);
                if (Config.SERVER_DEBUG)
                    c.sendMessage("Critical Strike threw: " + chance);
                if (chance <= c.stats[Stats.GetStatId("speed")] * 3){
                    return true;
                }
                else{
                    return false;
        ...
        public int CalculateCriticalDamage(int damage, String CombatStyle){
            if (CombatStyle.equalsIgnoreCase("melee"))              
        ...
        public void sendCriticalDamageGloating(String CombatStyle){
                if(CombatStyle.equalsIgnoreCase("Ranged")){
