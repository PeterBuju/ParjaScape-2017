...
public class PlayerAssistant {
    ...
    public void sendConstructMapRegion(Palette palette) {
        ...
        c.getOutStream().writeWordA(c.mapRegionY + 6);
        ...
        for (int z = 0; z < 4; z++) {
            for (int x = 0; x < 13; x++) {
                for (int y = 0; y < 13; y++) {
                    ...
                    if (tile != null) {
                        c.getOutStream().writeBits(26, tile.getX() << 14 | tile.getY() << 3 | tile.getZ() << 24 | tile.getRotation() << 1);
                    }
                }
            }
        }
    ...
    public void viewCookInterface(int item) {
        sendFrame164(1743);
        sendFrame246(13716, 190, item);
	...
	*
    ...
    public void playMusic2(int i, int j){
    ...
    public void walkTo(int i, int j) {
        ...
        if (i > 0){
            ...
            if (j > 0 && Region.blockedNorthEast(c.getX(), c.getY(), c.heightLevel)){
                i = 0;
            }
            else if (j < 0 && Region.blockedSouthEast(c.getX(), c.getY(), c.heightLevel)){
                i = 0;
            }
        }
        else if (i < 0){
            ...
            if (j > 0 && Region.blockedNorthWest(c.getX(), c.getY(), c.heightLevel)){
                i = 0;
            }
            else if (j < 0 && Region.blockedSouthWest(c.getX(), c.getY(), c.heightLevel)){
                i = 0;
            }
        }
        if (j > 0){
            ...
            if (i > 0 && Region.blockedNorthEast(c.getX(), c.getY(), c.heightLevel)){
                j = 0;
            }
            else if (i < 0 && Region.blockedNorthWest(c.getX(), c.getY(), c.heightLevel)){
                j = 0;
            }
        }
        if (j < 0){
            ...
            if (i > 0 && Region.blockedSouthEast(c.getX(), c.getY(), c.heightLevel)){
                j = 0;
            }
            else if (i < 0 && Region.blockedSouthWest(c.getX(), c.getY(), c.heightLevel)){
                j = 0;
            }
        }
        int k = c.getX() + i;
	...
	*
    ...
    public boolean addSkillXP(int amount, int skill) {
        ...
        if (Config.SERVER_DEBUG) {
            c.sendMessage("Stat name: " + Stats.GetStatName(Stats.GetStatIdOfSkill(skill)));
            c.sendMessage("Stat multiplier: " + c.stats[Stats.GetStatIdOfSkill(skill)]);
        }
        amount *= c.stats[Stats.GetStatIdOfSkill(skill)];
        if (Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("wisdom")) {
            amount *= Config.WISDOM_MULTIPLIER;
        } else if ((Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("constitution"))) {
            amount *= Config.CONSTITUTION_MULTIPLIER;
        } else if ((Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("endurance"))) {
            amount *= Config.ENDURANCE_MULTIPLIER;
        } else if ((Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("speed"))) {
            amount *= Config.SPEED_MULTIPLIER;
        } else if ((Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("intelligence"))) {
            amount *= Config.INTELLIGENCE_MULTIPLIER;
        } else if ((Stats.GetStatName(Stats.GetStatIdOfSkill(skill)).equalsIgnoreCase("dexterity"))) {
            amount *= Config.DEXTERITY_MULTIPLIER;
        }
        amount *= Config.SERVER_EXP_BONUS;
        if (Config.SERVER_DEBUG) {
            c.sendMessage("Gained: " + amount + " xp for skill " + skill);
        }
    ...
    public void MageStarter() {
        if (Config.SERVER_DEBUG) {
            c.sendMessage("You are now a @red@mage!");
        }
        c.finished = true;
    }

    public void ArcherStarter() {
        if (Config.SERVER_DEBUG) {
            c.sendMessage("You are now a @red@archer!");
        }
        c.finished = true;
    }

    public void WarriorStarter() {
        if (Config.SERVER_DEBUG) {
            c.sendMessage("You are now a @red@warrior!");
        }
        c.finished = true;
    }
    ...
    public void showMap() {
        int position = (c.getX() / 64 - 46) + (c.getY() / 64 - 49) * 6;
        c.getPA().sendFrame36(106, position);
        showInterface(5392);
    }
}
