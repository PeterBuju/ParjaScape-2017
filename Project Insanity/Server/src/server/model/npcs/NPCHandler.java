...
public class NPCHandler {
	...
	**/
	...
	public void process() {
                if (System.currentTimeMillis() - NPCGroupHandler.Timer > 5000){
                    NPCGroupHandler.ResetAttackerId();
                    NPCGroupHandler.Timer = System.currentTimeMillis();
                }
		for (int i = 0; i < maxNPCs; i++) {
                                ...
                                if (!npcs[i].description.equalsIgnoreCase("") && npcs[i].description != null){
                                    try{
                                        if (NPCGroupHandler.ContainsGroup(npcs[i].description)){
                                            ...
                                            if (group.getAttackerId() > 0)
                                            {
                                                npcs[i].killerId = group.getAttackerId();
                                                npcs[i].underAttack = true;
                                                npcs[i].underAttackBy = group.getAttackerId();
                                            }
                                        }
                                    }
                                    catch(Exception e){
						...
						if(Server.playerHandler.players[p] != null) {
							...
							if(npcs[i].attackTimer == 0) {
								if(c != null) {
                                                                        /*NPCGroup group = NPCGroupHandler.GetGroup(npcs[i].description);
                                                                        if (group != null){
                                                                            NPCGroupHandler.SetAttackerId(group.getName(), i);
                                                                        }*/
									attackPlayer(c, i);
								} else {
									npcs[i].killerId = 0;
		...
		while(EndOfFile == false && line != null) {
			...
			if (spot > -1) {
				...
				if (token.equals("spawn")) {
                                    if (token3.length == 9)
					newNPC(Integer.parseInt(token3[0]), Integer.parseInt(token3[1]), Integer.parseInt(token3[2]), Integer.parseInt(token3[3]), Integer.parseInt(token3[4]), getNpcListHP(Integer.parseInt(token3[0])), Integer.parseInt(token3[5]), Integer.parseInt(token3[6]), Integer.parseInt(token3[7]), token3[8]);
                                    else
