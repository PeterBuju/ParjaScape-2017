...
public class NPCHandler {
	...
	*
    ...
    public void newNPC(int npcType, int x, int y, int heightLevel, int WalkingType, int HP, int maxHit, int attack, int defence, String description) {
        // first, search for a free slot
        int slot = -1;
        for (int i = 1; i < maxNPCs; i++) {
            if (npcs[i] == null) {
        ...
        NPCGroupHandler.AddGroup(description);
        npcs[slot] = newNPC;
    }
                        ...
                        if (NPCGroupHandler.ContainsGroup(npcs[i].description)) {
                            ...
                            if (attacker != null) {
                                if (goodDistance(npcs[i].absX, npcs[i].absY, Server.playerHandler.players[attacker.id].absX, Server.playerHandler.players[attacker.id].absY, 3)) {
                                    ...
                                    npcs[i].underAttackBy = attacker.id;
                                    npcs[i].facePlayer(attacker.id);
                                }
                            }
                        }
                    } catch (Exception e) {
    ...
    public boolean retaliates(int npcType) {
        switch (NPCDataBase.FindDBNPC(npcType).name) {
            default:
                return true;
        }
    }
                ...
                if (token.equals("spawn")) {
                    if (token3.length == 9) {
                        if (Config.SERVER_DEBUG) {
                            //Misc.println(token3[0] + " - " + token3[8]);
                        }
                        DBNPC npc = NPCDataBase.FindDBNPC(Integer.parseInt(token3[0]));
                        if (npc == null) {
                            try {
                                npc = NPCDataBase.FindDBNPC(token3[8]);
                            } catch (Exception e) {

                            }
                        }
                        if (npc != null) {
                            newNPC(npc.id, Integer.parseInt(token3[1]), Integer.parseInt(token3[2]), Integer.parseInt(token3[3]), Integer.parseInt(token3[4]), getDBNpcHP(npc.id), Integer.parseInt(token3[5]), Integer.parseInt(token3[6]), Integer.parseInt(token3[7]), token3[8]);
                        }
                    } else {
                        try {
                            newNPC(Integer.parseInt(token3[0]), Integer.parseInt(token3[1]), Integer.parseInt(token3[2]), Integer.parseInt(token3[3]), Integer.parseInt(token3[4]), getDBNpcHP(Integer.parseInt(token3[0])), Integer.parseInt(token3[5]), Integer.parseInt(token3[6]), Integer.parseInt(token3[7]));
                        } catch (Exception e) {
    ...
    public int getDBNpcHP(int npcId) {
        DBNPC npc = NPCDataBase.FindDBNPC(npcId);
        if (Config.SERVER_DEBUG) {
            Misc.println(npc.name + "->" + npc.hp);
        }
        return npc.hp;
    }

    public String getDBNpcName(int npcId) {
        DBNPC npc = NPCDataBase.FindDBNPC(npcId);
        if (Config.SERVER_DEBUG) {
            Misc.println(npc.name + "->" + npc.hp);
        }
    ...
    public boolean facingPlayer(NPC npc, Client c){
        ...
        if (npc.absY < c.absY){ //PLAYER IS NORTHERN
            ...
            else if (npc.absX > c.absX){ //PLAYER IS TO NORTH-WEST
                if (direction == 13 || direction == 14 || direction == 15){
                    return true;
                }
            }
            else if (npc.absX < c.absX){ //PLAYER IS TO NORTH-EAST
                if (direction == 1 || direction == 2 || direction == 3){
                    return true;
                }
            }
        }
        else if (npc.absY == c.absY){ //PLAYER IS AT THE SAME Y
            if (npc.absX == c.absX){ //PLAYER IS ON TOP OF NPC
                if (direction == -1){
                    return true;
                }
            }
            else if (npc.absX > c.absX){ //PLAYER IS TO WEST
                if (direction == 11 || direction == 12 || direction == 13){
                    return true;
                }
            }
            else if (npc.absX < c.absX){ //PLAYER IS TO EAST
                if (direction == 3 || direction == 4 || direction == 5){
                    return true;
                }
            }   
        }
        else if (npc.absY > c.absY){ //PLAYER IS SOUTHERN
            if (npc.absX == c.absX){ //PLAYER IS TO SOUTH
                if (direction == 7 || direction == 8 || direction == 9){
                    return true;
                }
            }
            else if (npc.absX > c.absX){ //PLAYER IS TO SOUTH-WEST
                if (direction == 9 || direction == 10 || direction == 11){
                    return true;
                }
            }
            else if (npc.absX < c.absX){ //PLAYER IS TO SOUTHEAST
