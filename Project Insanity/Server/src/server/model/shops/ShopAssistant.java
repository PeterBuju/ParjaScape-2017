...
public class ShopAssistant {
	...
	**/
	...
	public void resetShop(int ShopID) {
        ...
        public double CharismaModifier(double price, int Type){
            int charisma = c.stats[6];
            double modifier = charisma * 0.015;
            if (Type == 0) //BUYING
            {
                price *= Config.BUYMODIFIER - modifier;
            }
            else if (Type ==1) //SELLING
            {
                price *= Config.SELLMODIFIER + modifier;
            }
            if (Config.SERVER_DEBUG)
                c.sendMessage("New Price: " + price + " ||Modifier: " + modifier + " || Charisma: " + charisma);
            return price;
        }
	...
	public double getItemShopValue(int ItemID, int Type, int fromSlot) {
		...
		if (Server.shopHandler.ShopBModifier[c.myShopId] == 1) {
			...
			TotPrice *= 1;
                ...
                TotPrice = CharismaModifier(TotPrice, Type);
		return TotPrice;
	}
	...
	**/
	...
	public void buyFromShopPrice(int removeId, int removeSlot){
		int ShopValue = (int)Math.ceil(getItemShopValue(removeId, 0, removeSlot));
		String ShopAdd = "";
		if (c.myShopId >= 17) {
	...
	**/
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			...
			for (int i = amount; i > 0; i--) {
				TotPrice2 = (int)Math.ceil(getItemShopValue(itemID, 0, fromSlot));
                ...
                if(c.myShopId != 29 || c.myShopId != 30 || c.myShopId != 31 || c.myShopId != 47) {
					if (c.playerItemsN[Slot] >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
                                                        if (c.perks.contains(PerkHandler.GetPerk("Golden Tongue")))
                                                        {
                                                            Random rand = new Random();
                                                            int chance = Misc.random(100);
                                                            if (Config.SERVER_DEBUG)
                                                                c.sendMessage("GoldenTogue threw "+chance);
                                                            if (chance <= 3)
                                                                c.sendMessage("The shopkeeper was so charmed that he didn't ask you any money");
                                                            else
