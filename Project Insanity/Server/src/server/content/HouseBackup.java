...
public class HouseBackup implements Serializable {
    ...
    public HouseBackup getHouse(Client player) {
        HouseBackup d = null;
        try {
            ObjectInputStream in = new ObjectInputStream(new FileInputStream(houseLocation + player.playerName + ".house"));
            d = (HouseBackup) in.readObject();
            in.close();
            System.out.println("umm:" + d.objects.size());
        } catch (Exception e) {
            //	e.printStackTrace();
            return new HouseBackup(player, false);
        }
        return d;

    }

    public HouseBackup(Client player, boolean hasHouse) {
        for (int x = 0; x < 11; x++) {
            for (int y = 0; y < 11; y++) {
                //rooms[x][y][0] = this.rooms[x][y][0];
                rooms[x][y][0] = GARDEN;
