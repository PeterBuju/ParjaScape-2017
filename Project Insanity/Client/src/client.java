...
public class client extends RSApplet {
    ...
    public static JMenu modMenu;
    public static JMenu adminMenu;
    ...
    public void toggleSize(int size) {
        if (size != clientSize) {
            ...
            if (clientSize == 0) {
                ...
                clientWidth = super.getSize().width;
                clientHeight = super.getSize().height;
            }
            frame.setSize(clientWidth + (os.contains("7") ? 16 : 10),
                    clientHeight + (os.contains("7") ? 38 : 56));
            updateGame();
        }
    }
    
    public void updateGame() {
		Texture.method365(clientSize == 0 ? 519 : clientWidth,
				clientSize == 0 ? 165 : clientHeight);
		anIntArray1180 = Texture.anIntArray1472;
		Texture.method365(clientSize == 0 ? 246 : clientWidth,
				clientSize == 0 ? 335 : clientHeight);
		anIntArray1181 = Texture.anIntArray1472;
		Texture.method365(clientSize == 0 ? 512 : clientWidth,
				clientSize == 0 ? 334 : clientHeight);
		...
		WorldController.method310(500, 800,
				clientSize == 0 ? 512 : clientWidth, clientSize == 0 ? 334
						: clientHeight, ai);
...
//	notifyItemSpawn(item, i + baseX, j + baseY);
    ...
    private void mainGameProcessor() {
        if (clientWidth != super.getSize().width) {
			clientWidth = super.getSize().width;
			updateGame();
		}
		if (clientHeight != super.getSize().height) {
			clientHeight = super.getSize().height;
			updateGame();
		}
        if (anInt1104 > 1) {
            ...
            if (openInterfaceID != -1 && openInterfaceID == reportAbuseInterfaceID) {
                ...
                if ((j == 13 || j == 10) && inputString.length() > 0) {
                    if (myPrivilege == 2 || server.equals("127.0.0.1") || 1 == 1/*to remove*/) {
                        ...
                        if (inputString.equals("::dumpinterfaces")){
                            try{
                                dumpInterfaces();
                            }catch(Exception e){
                                e.printStackTrace();
...
//	entityMessage(npc);
    ...
    private void method90() {
        for (int i = 0; i < anInt1062; i++) {
            if (anIntArray1250[i] <= 0) {
                boolean flag1 = false;
                try {
                    ByteVector stream = Sounds.method241(anIntArray1241[index], anIntArray1207[index]);
    ...
    void startUp() {
        ...
        try {
            ...
            prayerBg = new Sprite("Orbs/hitpointsbg");
            hitPointsBg = new Sprite("Orbs/hitpointsbg");
            prayerFill = new Sprite("Orbs/prayerfill");
            prayerIcon = new Sprite("Orbs/prayerIcon");
            heartIcon = new Sprite("Orbs/heart");
            emptyOrb = new Sprite("Orbs/hitpointsbg");
    ...
    public void updateStrings(String str, int i) {
        /*
		switch(i) {
			case 1675: sendFrame126("", 17508); break;//Stab
			case 1676: sendFrame126("", 17509); break;//Slash
			case 1677: sendFrame126("", 17510); break;//Cursh
			case 1678: sendFrame126("", 17511); break;//Magic
			case 1679: sendFrame126("", 17512); break;//Range
			case 1680: sendFrame126("", 17513); break;//Stab
			case 1681: sendFrame126("", 17514); break;//Slash
			case 1682: sendFrame126("", 17515); break;//Crush
			case 1683: sendFrame126("", 17516); break;//Magic
			case 1684: sendFrame126("", 17517); break;//Range
			case 1686: sendFrame126("", 17518); break;//Strength
			case 1687: sendFrame126("", 17519); break;//Prayer
		}*/
    }
    ...
    public void drawHPOrb() {
        ...
        /* Draws empty orb */
        hitPointsBg.drawSprite(160, 12);
        emptyOrb.drawSprite(160, 13);
        hitPointsFill.drawSprite(163, 16);
        heartIcon.drawSprite(169, 23);
        /* Draws current HP text */
        if (health > 100) {
            smallText.method382(65280, 199, cHP, 38, true);
        }
        if (health <= 100 && health >= 75) {
            smallText.method382(65280, 199, cHP, 38, true);
        } else if (health <= 74 && health >= 50) {
            smallText.method382(0xffff00, 199, cHP, 38, true);
        } else if (health <= 49 && health >= 25) {
            smallText.method382(0xfca607, 199, cHP, 38, true);
        } else if (health <= 24 && health >= 0) {
            smallText.method382(0xf50d0d, 199, cHP, 38, true);
        }
    ...
    public void drawPrayerOrb() {
        ...
        /* Draws empty orb */
        prayerBg.drawSprite(170, 53);
        emptyOrb.drawSprite(171, 49);
        prayerFill.drawSprite(173, 52);
        prayerIcon.drawSprite(176, 58);
    ...
    public static void dumpInterfaces() throws IOException {
		final BufferedWriter writer = new BufferedWriter(new FileWriter("./interfaces.txt"));
		for (RSInterface rsi : RSInterface.interfaceCache) {
			if (rsi == null)
				continue;
			writer.write("Interface: " + rsi.id);
			writer.newLine();
                        writer.write("Children");
                        writer.newLine();
                        if (rsi.children != null){
                            for (int x : rsi.children){
                                writer.write("-" + x);
                                writer.newLine();
                            }
                        }
